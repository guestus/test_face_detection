...
class FaceRecognitionThread(threading.Thread):
    ...
    def _mainloop_(self):
        self.do_init()
        self.face = cv2.dnn.readNetFromTensorflow('website/facerecognition/checkpoint/opencv_face_detector_uint8.pb','website/facerecognition/checkpoint/opencv_face_detector.pbtxt')
        self.bb = cv2.dnn.readNetFromONNX('website/facerecognition/checkpoint/model.onnx')
        if self.needPreload:
            do_preload()
        facenum=0
