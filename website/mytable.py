...
mytable = Blueprint('mytable', __name__)
...
@mytable.route('/video_feed')
@login_required
def video_feed():
    return Response(gen_frames(), mimetype='multipart/x-mixed-replace; boundary=frame')

@mytable.route('/table', methods=['GET', 'POST'])
@login_required
def getmytable():
    return render_template("table.html", user=current_user, thread=current_thread().ident)